  ...
  const submit = async () => {
    try {
      ...
      if (response.data.role !== "admin") {
        toast.error("Access Denied: Only admin have admin has access");
        return;
      }

      // Dispatch login actions if the role is admin
        ...
        <div className="mt-4">
          ...
          <div className="mt-4">
            ...
            <div className="flex bg-[#e8f0fe] rounded-md">
              ...
              >
                {showPassword ? (
                  <EyeOff className="text-zinc-900 text-lg mr-2" />
                ) : (
                  <Eye className="text-zinc-900 text-lg mr-2" />
                )}
              </button>
            </div>
          </div>
